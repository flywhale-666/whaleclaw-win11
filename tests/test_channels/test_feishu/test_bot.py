...
def test_prepare_reply_payload_extracts_windows_pptx_bare_path(tmp_path: Path) -> None:
    pptx = tmp_path / "windows-demo.pptx"
    pptx.write_bytes(b"pptx")
    windows_path = str(pptx.resolve())
    bot = FeishuBot(_StubClient(), FeishuConfig(dm_policy="open"))

    text, images, files = bot._prepare_reply_payload(  # noqa: SLF001
        f"PPT 文件路径: {windows_path}"
    )

    assert "📎 windows-demo.pptx" in text
    assert images == []
    assert files == [pptx]


def test_prepare_reply_payload_extracts_windows_markdown_file_path(tmp_path: Path) -> None:
    pptx = tmp_path / "windows-link.pptx"
    pptx.write_bytes(b"pptx")
    windows_path = str(pptx.resolve())
    bot = FeishuBot(_StubClient(), FeishuConfig(dm_policy="open"))

    text, images, files = bot._prepare_reply_payload(  # noqa: SLF001
        f"[下载 PPT]({windows_path})"
    )

    assert "📎 下载 PPT" in text
    assert images == []
    assert files == [pptx]
