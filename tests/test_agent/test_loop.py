...
def test_build_nano_banana_command_includes_api_key() -> None:
    ...
    assert "sk-test-key" in command
    assert "--api-key" not in command
    assert "NANO_BANANA_API_KEY" in command
    assert "--base-url" in command
    assert "https://ai.t8star.cn" in command
    assert "--out-dir" in command
    assert "nano_banana_test" in command
...
def test_skill_helpers_support_clean_chinese_activation_and_completion() -> None:
    ...
    assert is_skill_switch_consent(
        "同意切换技能",
        skill_switch_consent_patterns=loop_mod._SKILL_SWITCH_CONSENT_PATTERNS,  # noqa: SLF001
    )
