...
from whaleclaw.agent.multi_agent import (
    _capture_recent_delivery_artifacts,
    build_multi_agent_final_reply,
)
...
class _StubSessionManager:
    ...
    async def update_metadata(self, session: object, metadata: object) -> None:
...
def test_build_multi_agent_final_reply_includes_round_details() -> None:
    reply = build_multi_agent_final_reply(
        [
            "### 第 1 轮角色观点\n\n## 第 1 轮交付\n1) 产物A",
            "### 第 2 轮角色观点\n\n## 第 2 轮交付\n1) 产物B",
        ],
        aborted_round_no=None,
    )

    assert "多Agent执行完成，共 2 轮。" in reply
    assert "[第1轮结果]" in reply
    assert "产物A" in reply
    assert "[第2轮结果]" in reply
    assert "产物B" in reply



def test_capture_recent_delivery_artifacts_finds_recent_pptx(tmp_path: Path) -> None:
    recent = tmp_path / "deck_V3.pptx"
    recent.write_bytes(b"pptx")
    old = tmp_path / "old_deck.pptx"
    old.write_bytes(b"old")
    old_ts = time.time() - 600
    os.utime(old, (old_ts, old_ts))

    out = _capture_recent_delivery_artifacts(
        since_ts=time.time() - 10,
        roots=(tmp_path,),
    )

    assert str(recent.resolve()) in out
    assert str(old.resolve()) not in out
