...
const _LOCAL_PATH_RE = /^(?:\/|~\/|\.\/|\.\.\/|[A-Za-z]:[\\/]|\\\\)/;
const _UNIX_FILE_PATH_GLOBAL_RE = /(\/(?:tmp|home|Users|var|opt|etc)\/[^\n<"']+?\.(\w{2,5}))(?=[\s<"']|$)/gi;
const _WINDOWS_FILE_PATH_GLOBAL_RE = /([A-Za-z]:[\\/][^\n<"']+?\.(\w{2,5}))(?=[\s<"']|$)/g;
const _WINDOWS_MD_IMAGE_RE = /!\[([^\]]*)\]\(((?:[A-Za-z]:[\\/]|\\\\)[^)]+)\)/g;
const _WINDOWS_MD_LINK_RE = /\[([^\]]+)\]\(((?:[A-Za-z]:[\\/]|\\\\)[^)]+)\)/g;
const _NON_DELIVERY_FILE_EXTS = new Set(['py', 'sh', 'bash', 'zsh', 'log', 'tmp']);
...
md.renderer.rules.image = function (tokens, idx, options, env, self) {
  ...
  if (srcIdx >= 0) {
    ...
    if (src && _LOCAL_PATH_RE.test(src) && !src.startsWith('/api/')) {
...
createApp({
  setup() {
    ...
    /*
    async function doClawhubLoginLegacy() {
    ...
    */
    async function doClawhubLogin() {
      if (!clawhubCliAvailable.value) {
        showUiAlert('璇峰厛瀹夎 CLI锛屽啀鐧诲綍');
        return;
      }
      clawhubLoggingIn.value = true;
      try {
        const data = await apiFetch('/api/clawhub/login', { method: 'POST' });
        if (!data.ok || !data.auth_url || !data.state) {
          showUiAlert(data.message || '登录初始化失败');
          return;
        }
        window.open(data.auth_url, '_blank', 'noopener,noreferrer');
        const deadline = Date.now() + ((Number(data.expires_in) || 180) * 1000);
        while (Date.now() < deadline) {
          await sleep(2000);
          const poll = await apiFetch(`/api/clawhub/login-poll?state=${encodeURIComponent(data.state)}`);
          if (!poll.done) continue;
          if (!poll.ok) {
            showUiAlert(poll.message || '登录失败，请退出后重试');
          }
          break;
        }
        await loadClawhubAuthStatus();
    ...
    /*
    async function doClawhubReloginLegacy() {
      return;
    }

    async function doClawhubLoginDebug() {
      if (!clawhubCliAvailable.value) {
        showUiAlert('请先安装 CLI，再登录 ClawHub');
        return;
      }
      clawhubLoggingIn.value = true;
      try {
        ...
        if (!popup) {
          showUiAlert('浏览器拦截了登录弹窗，请允许弹窗后重试');
          return;
        }
        const deadline = Date.now() + ((Number(data.expires_in) || 180) * 1000);
        let finished = false;
        ...
        if (!finished) {
          showUiAlert('未收到登录回调，请确认浏览器已完成 ClawHub 登录，并允许访问本地回调地址');
        }
        await loadClawhubAuthStatus();
      } catch (e) {
        showUiAlert('登录失败: ' + (e.message || e));
    ...
    */
    ...
    function _normalizeLocalFilePath(filePath) {
      let normalized = String(filePath || '').trim();
      if (!normalized) return normalized;
      normalized = normalized
        .replace(/([A-Za-z]:\\Users\\[^\\]+)\.(whaleclaw|openclaw|codex|local)(?=\\)/gi, '$1\\.$2')
        .replace(/([A-Za-z]:\\[^\\]+)\.(whaleclaw|openclaw|codex|local)(?=\\)/gi, '$1\\.$2');
      return normalized;
    }

    function _fileCard(filePath) {
      const normalizedPath = _normalizeLocalFilePath(filePath);
      const name = normalizedPath.split(/[\\/]/).pop() || normalizedPath;
      const ext = (name.split('.').pop() || '').toLowerCase();
      const icon = _FILE_ICONS[ext] || '📎';
      const encPath = encodeURIComponent(normalizedPath);
    ...
    function renderMarkdown(text) {
      if (!text) return '';
      const source = text
        .replace(_WINDOWS_MD_IMAGE_RE, (_m, alt, filePath) => `![${alt}](/api/local-file?path=${encodeURIComponent(filePath)})`)
        .replace(_WINDOWS_MD_LINK_RE, (_m, label, filePath) => `[${label}](/api/local-file?path=${encodeURIComponent(filePath)})`);
      ...
      html = html.replace(
        ...
        (m, pre, src, post) => {
          ...
          if (!_LOCAL_PATH_RE.test(src)) return m;
      ...
      html = html.replace(
        _UNIX_FILE_PATH_GLOBAL_RE,
        (m, filePath, ext) => {
          if (_IMAGE_EXTS.has(ext.toLowerCase())) return m;
          if (_NON_DELIVERY_FILE_EXTS.has(ext.toLowerCase())) return m;
          if (m.includes('file-card')) return m;
          return _fileCard(filePath);
        }
      );
      html = html.replace(
        _WINDOWS_FILE_PATH_GLOBAL_RE,
        (m, filePath, ext) => {
          if (_IMAGE_EXTS.has(ext.toLowerCase())) return m;
          if (_NON_DELIVERY_FILE_EXTS.has(ext.toLowerCase())) return m;
          if (m.includes('file-card')) return m;
