...
def create_app(config: WhaleclawConfig) -> FastAPI:
    ...
    @app.get("/api/clawhub/login-poll")
    async def _api_clawhub_login_poll(state: str = "") -> JSONResponse:
        login_state = state.strip()
        if not login_state:
            return JSONResponse({"error": "缺少 state 参数"}, status_code=400)

        cfg = _read_clawhub_cfg()
        try:
            result = poll_browser_login(
                state=login_state,
                registry_url=str(cfg["registry_url"]),
                workspace_dir=WORKSPACE_DIR,
                api_token=str(cfg["api_token"]) or None,
            ...
            return JSONResponse({"error": f"登录轮询失败: {exc}"}, status_code=500)
        return JSONResponse(
            {
    ...
    async def _api_add_cron_job(body: dict[str, Any]) -> JSONResponse:
        ...
        job = CronJob(
            ...
            name=auto_name,
            schedule_obj=sched,
            action=CronAction(type="agent", target=action_target, payload={"text": message}),
            enabled=True,
            created_at=now,
            one_shot=one_shot,
        )
    ...
    def _resolve_local_path(path: str) -> Path | None:
        ...
        decoded = unquote(unquote(path)).strip()
        candidate_texts = [decoded]
        hidden_dir_variants = (".whaleclaw", ".openclaw", ".codex", ".local")
        for variant in hidden_dir_variants:
            normalized = _re.sub(
                rf"(?<=[^\\/]){_re.escape(variant)}(?=[\\/])",
                rf"\\{variant}",
                decoded,
            )
        ...
        for candidate_text in candidate_texts:
            ...
            fp = Path(candidate_text).resolve()
            if fp.is_file():
                return fp
            stem = fp.stem
