...
ABS_FILE_PATH_RE = re.compile(
    r"((?:/[^\s\"')]+|[A-Za-z]:[\\/][^\s\"')]+?)\.[A-Za-z0-9]{1,8})(?=[\s\"')]|$)"
)
...
def next_available_version_path(path: str) -> str:
    target = Path(str(Path(path).expanduser()))
    stem = target.stem
    ...
    version = 1
    while True:
        candidate = target.with_name(f"{base_stem}_V{version}{target.suffix}")
...
def fix_version_suffix(paths: list[str], round_no: int) -> tuple[list[str], dict[str, str]]:
    ...
    for path in paths:
        ...
        stem = file_path.stem
        match = VERSION_SUFFIX_RE.search(stem)
        needs_fix = match is None or match.group(0).upper() != expected.upper()
        if needs_fix:
            correct = with_round_version_suffix(path, round_no)
            correct_path = Path(correct)
            try:
                correct_path.parent.mkdir(parents=True, exist_ok=True)
                if correct_path.resolve() != file_path.resolve():
...
def build_image_generation_system_message() -> Message:
    return Message(
        ...
        content=(
            ...
            "2) 之后必须立即写最小脚本到 ~/.whaleclaw/workspace/tmp 并执行一次真实请求；\n"
