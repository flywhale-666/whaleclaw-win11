...
_BROWSER_LOGIN_TIMEOUT_SECONDS = 180.0
_BROWSER_LOGIN_VERIFY_GRACE_SECONDS = 15.0
...
class _BrowserLoginSession:
    ...
    token_saved_at: float = 0.0


_browser_login_sessions: dict[str, _BrowserLoginSession] = {}
...
"""
def start_browser_login(*, registry_url: str) -> dict[str, object]:
    """Start the browser callback login flow and return the auth URL."""
    if not is_clawhub_cli_available():
        raise ClawHubCliError("CLI 未安装，请先点击“安装 CLI”")

    _cleanup_expired_browser_login_sessions()
    state = secrets.token_urlsafe(24)
    server, redirect_uri = _create_browser_callback_server(state)
    auth_url = (
        f"{registry_url.rstrip('/')}/cli/auth"
        f"?redirect_uri={quote(redirect_uri, safe='')}"
        f"&state={quote(state, safe='')}"
    )
    session = _BrowserLoginSession(
        state=state,
        auth_url=auth_url,
        redirect_uri=redirect_uri,
        expires_at=time.time() + _BROWSER_LOGIN_TIMEOUT_SECONDS,
        server=server,
    )
    with _browser_login_lock:
        _browser_login_sessions[state] = session
    _start_browser_callback_server(server)
    return {
        "ok": True,
        "state": state,
        "auth_url": auth_url,
        "redirect_uri": redirect_uri,
        "expires_in": int(_BROWSER_LOGIN_TIMEOUT_SECONDS),
    }


def poll_browser_login(
    *,
    state: str,
    registry_url: str,
    workspace_dir: Path,
    ...
    """Poll a browser login session until the CLI can verify the token."""
    _cleanup_expired_browser_login_sessions()
    session = _get_browser_login_session(state)
    if session is None:
        return {
            "done": True,
            "ok": False,
            "message": "登录会话不存在或已过期，请重新登录。",
        }

    now = time.time()
    if session.status == "success":
        return {"done": True, "ok": True, "message": session.message}
    if session.status == "error":
        return {"done": True, "ok": False, "message": session.message}

    if session.token_saved_at <= 0:
        if now >= session.expires_at:
            message = "登录超时，请重新点击“登录 ClawHub”。"
            _set_browser_login_status(state, status="error", message=message)
            return {"done": True, "ok": False, "message": message}
        return {"done": False, "ok": False, "message": session.message}
    ...
    if bool(status["logged_in"]):
        message = str(status["message"])
        _set_browser_login_status(state, status="success", message=message)
        return {"done": True, "ok": True, "message": message}

    if now - session.token_saved_at < _BROWSER_LOGIN_VERIFY_GRACE_SECONDS:
        return {"done": False, "ok": False, "message": "已收到登录令牌，正在校验登录状态..."}

    message = str(status["message"]) or "登录失败，请退出后重新登录。"
    _set_browser_login_status(state, status="error", message=message)
    return {"done": True, "ok": False, "message": message}
...
def install_clawhub_cli() -> dict[str, str]:
    ...
    npm_bin = _resolve_npm_bin()
    if not npm_bin:
        raise ClawHubCliError("npm not found; install Node.js first")
    ...
    status = get_clawhub_cli_status()
    if not bool(status["available"]):
        raise ClawHubCliError("install finished but clawhub executable was not found")
    return {
        "path": str(status["path"]),
        "version": str(status["version"]),
        "output": stdout,
...
def get_clawhub_auth_status(
    *,
    ...
    """Return CLI auth status by running ``clawhub whoami``."""
    if not is_clawhub_cli_available():
        return {"logged_in": False, "message": "CLI not installed"}
    env = _build_env(
        registry_url=registry_url,
        workspace_dir=workspace_dir,
        api_token=api_token,
...
def login_clawhub_cli(
    *,
    ...
    """Run ``clawhub login`` and return post-login status."""
    if not is_clawhub_cli_available():
        raise ClawHubCliError("CLI not installed. Install the CLI first.")
    env = _build_env(
        registry_url=registry_url,
        workspace_dir=workspace_dir,
        api_token=api_token,
    ...
    *,
    ...
    if session is None:
        return {
            "done": True,
            "ok": False,
            "message": "Login session missing or expired. Start login again.",
        }
    ...
    if session.token_saved_at <= 0:
        if now >= session.expires_at:
            message = "Login timed out. Start the ClawHub login flow again."
            _set_browser_login_status(state, status="error", message=message)
            return {"done": True, "ok": False, "message": message}
        return {"done": False, "ok": False, "message": session.message}
    ...
    if now - session.token_saved_at < _BROWSER_LOGIN_VERIFY_GRACE_SECONDS:
        return {"done": False, "ok": False, "message": "Token received. Verifying login state..."}

    message = str(status["message"]) or "Login failed. Log out and try again."
    _set_browser_login_status(state, status="error", message=message)
    return {"done": True, "ok": False, "message": message}
...
def _create_browser_callback_server(state: str) -> tuple[ThreadingHTTPServer, str]:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.bind(("127.0.0.1", 0))
        port = int(sock.getsockname()[1])
    server = ThreadingHTTPServer(("127.0.0.1", port), _ClawHubCallbackHandler)
    server.daemon_threads = True
    setattr(server, "clawhub_login_state", state)
    return server, f"http://127.0.0.1:{port}/callback"


def _start_browser_callback_server(server: ThreadingHTTPServer) -> None:
    thread = threading.Thread(
        target=server.serve_forever,
        kwargs={"poll_interval": 0.2},
        daemon=True,
        name="clawhub-login-callback",
    )
    thread.start()


def _get_browser_login_session(state: str) -> _BrowserLoginSession | None:
    with _browser_login_lock:
        return _browser_login_sessions.get(state)


def _set_browser_login_status(state: str, *, status: str, message: str) -> None:
    with _browser_login_lock:
...
def _mark_browser_login_token_received(state: str, token: str) -> None:
    _write_clawhub_global_token(token)
    with _browser_login_lock:
        session = _browser_login_sessions.get(state)
        if session is None:
            return
        session.status = "token_received"
        session.message = "已收到登录令牌，正在校验登录状态..."
        session.token_saved_at = time.time()
        server = session.server
        session.server = None
    if server is not None:
        _shutdown_browser_callback_server(server)


def _cleanup_expired_browser_login_sessions() -> None:
    now = time.time()
    expired_servers: list[ThreadingHTTPServer] = []
    with _browser_login_lock:
        expired_states = [
            state
        ...
        for state in expired_states:
            session = _browser_login_sessions.pop(state)
            if session.server is not None:
                expired_servers.append(session.server)
    for server in expired_servers:
        _shutdown_browser_callback_server(server)


def _shutdown_browser_callback_server(server: ThreadingHTTPServer) -> None:
    with contextlib.suppress(Exception):
        server.shutdown()
    with contextlib.suppress(Exception):
        server.server_close()
...
def _write_clawhub_global_token(token: str) -> None:
...
class _ClawHubCallbackHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        parsed = urlparse(self.path)
        if parsed.path not in {"/", "/callback", "/auth/callback"}:
            self.send_error(404)
            return
        ...
        session = _get_browser_login_session(state)
        if session is None:
            self.send_error(410, "Login session expired")
            return

        self.send_response(200)
        self.send_header("Content-Type", "text/html; charset=utf-8")
        self.end_headers()
        self.wfile.write(_build_clawhub_callback_html(expected_state=state).encode("utf-8"))
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        session = _get_browser_login_session(state)
        if session is None:
            self._write_json(410, {"ok": False, "message": "登录会话已过期"})
            return
        ...
        posted_state = str(payload.get("state", "")).strip() or state
        token = str(
            payload.get("token")
        ...
        if posted_state != state:
            self._write_json(400, {"ok": False, "message": "state mismatch"})
            return
        if not token:
            self._write_json(400, {"ok": False, "message": "missing token"})
            return

        try:
            _mark_browser_login_token_received(state, token)
        except Exception as exc:
            _set_browser_login_status(state, status="error", message=f"保存登录令牌失败: {exc}")
            self._write_json(500, {"ok": False, "message": f"保存登录令牌失败: {exc}"})
            return

        self._write_json(200, {"ok": True, "message": "token received"})

    def log_message(self, format: str, *args: object) -> None:  # noqa: A002
        return

    def _server_state(self) -> str:
        return str(getattr(self.server, "clawhub_login_state", ""))
    ...
    def _read_payload(self) -> dict[str, object]:
        ...
        return {
            str(key): values[-1] if values else ""
...
def _build_clawhub_callback_html(*, expected_state: str) -> str:
    safe_state = html.escape(expected_state, quote=True)
    return f"""<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>ClawHub 登录</title>
</head>
<body>
  <p id="status">正在接收 ClawHub 登录结果...</p>
  <script>
    ...
    const token = query.get('token') || query.get('access_token') || query.get('id_token') || hash.get('token') || hash.get('access_token') || hash.get('id_token') || '';
    const state = query.get('state') || hash.get('state') || expectedState;
    const statusEl = document.getElementById('status');

    async function submitToken() {{
      if (!token) {{
        statusEl.textContent = '登录回调缺少 token，请关闭后重试。';
        return;
      }}
      try {{
        ...
        if (!resp.ok) {{
          ...
          statusEl.textContent = text || '登录失败，请返回 WhaleClaw 重试。';
          return;
        }}
        statusEl.textContent = '登录成功，已返回 WhaleClaw。现在可以关闭此页面。';
        window.close();
      }} catch (error) {{
        statusEl.textContent = '登录结果提交失败，请返回 WhaleClaw 重试。';
      }}
    }}
