...
3. `file_write` → 写完整 Python 脚本到临时工作目录（优先 `~/.whaleclaw/workspace/tmp/gen_xxx.py`）
4. `bash` → 执行脚本（Windows 优先 `.\python\python.exe`，mac/Linux 优先 `./python/bin/python3.12`）
5. 告诉用户文件路径
...
调用示例：`add_picture_cropped(slide, "/absolute/path/photo.jpg", Inches(1), Inches(1.5), Inches(5), Inches(4))`
**封面底图遮挡（必须遵守）：**
...
- 保存到临时工作目录（优先 `~/.whaleclaw/workspace/tmp/<有意义的文件名>.<后缀>`）
- 文件名必须去除空格与特殊符号，仅保留中文/英文/数字/下划线，例如：`~/.whaleclaw/workspace/tmp/同安2日游.pptx`
